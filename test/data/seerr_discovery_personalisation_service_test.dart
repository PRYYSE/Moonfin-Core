...
void main() {
  test('uses stable accepted Home row slot and dynamic generated title', () async {
    int? requestedSlot;
  ...
  test('movie series and anime sections constrain the accepted engine', () async {
  ...
  test('pages through the existing scored Home recommendation cache', () async {
    ...
    expect(first.page.results.map((item) => item.id),
        List.generate(15, (index) => index + 1));
    expect(second.page.results.map((item) => item.id),
        List.generate(15, (index) => index + 16));
    expect(third.page.results.map((item) => item.id), [31, 32]);
    expect(loadMoreCalls, 2);
  });

  test('local accepted recommendation is surfaced as available with Jellyfin id',
      () async {
    final service = SeerrDiscoveryPersonalisationService.forTesting(
      serverId: 'server-1',
      loadRow: (
