...
void main() {
  test('expanded external query preserves configured source order across pages',
      () async {
    ...
    expect(first.results.map((item) => item.id),
        List.generate(20, (index) => index + 1));
    expect(second.results.map((item) => item.id),
  ...
  test('expanded query rejects non-external source instead of broadening it',
      () async {
    final service = SeerrDiscoveryConfiguredListsService.forTesting(
      readConfigs: () => [_config()],
