...
void main() {
  test(
    'defaults to For You and uses the accepted generated personal title',
    () async {
  ...
  test('general discovery retains available local media', () async {
    ...
    final available = vm.rows.firstWhere(
      (row) => row.section.id == 'available',
    );
  ...
  test('requestable lane excludes already available media', () async {
    ...
    await vm.load();
    await vm.selectTab('movies');
    final row = vm.rows.firstWhere(
      (entry) => entry.section.id == 'requestable',
    );
  ...
  test(
    'existing NSFW preference still filters dynamic catalogue results',
    () async {
      final vm = SeerrDeepDiscoveryViewModel.forTesting(
        loadCatalogue: () async => _loadResult(),
        loadSessionSeed: () async => 'user-1',
  ...
  test(
    'unconfigured external list lane fails closed without breaking tab',
    () async {
      final vm = SeerrDeepDiscoveryViewModel.forTesting(
        loadCatalogue: () async => _loadResult(),
        loadSessionSeed: () async => 'user-1',
  ...
  test(
    'loadMore appends personal pages without duplicating existing ids',
    () async {
      final vm = SeerrDeepDiscoveryViewModel.forTesting(
        loadCatalogue: () async => _loadResult(),
        loadSessionSeed: () async => 'user-1',
