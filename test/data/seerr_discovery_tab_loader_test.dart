...
void main() {
  test(
    'preserves session order while limiting concurrent lane fetches',
    () async {
      var inFlight = 0;
      var maxInFlight = 0;
  ...
  test(
    'failed optional lane is omitted while a failed anchor remains retryable',
    () async {
      ...
      expect(result.failedOptional.map((entry) => entry.section.id), [
        'optional',
      ]);
    },
  );
  ...
  test(
    'successful shallow lane is classified separately from an error',
    () async {
      final shallow = _section('shallow', minItems: 2);
      final loader = SeerrDiscoveryLaneLoader(
        fetchPage: (_, __) async =>
            SeerrDiscoverPage(page: 1, totalPages: 1, results: [_item(1)]),
      );
