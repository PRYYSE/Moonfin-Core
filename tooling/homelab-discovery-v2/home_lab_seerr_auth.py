#!/usr/bin/env python3
"""Read-only Seerr API access for Home Lab deployment validation."""
...
def _seerr_container() -> str:
    names = [
        name.strip()
        for name in _docker(["ps", "--format", "{{.Names}}"]).__str__().splitlines()
        if name.strip()
    ]
    matches = [
        name
...
def _container_metadata(name: str) -> dict[str, Any]:
...
def _api_key_from_environment(metadata: dict[str, Any]) -> str | None:
...
def _api_key_from_settings(metadata: dict[str, Any]) -> str:
...
def _configured_seerr_url(
    jellyfin_token: str,
    request_json: Callable[[str, str, str], Any],
) -> str:
