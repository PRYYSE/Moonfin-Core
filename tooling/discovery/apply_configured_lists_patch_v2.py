...
This revision targets the already formatted staging source and is idempotent.
"""
...
def patch_vm(text: str) -> str:
    if "SeerrDeepCatalogueMerger _mergeCatalogue" in text:
        return text
    ...
    text = replace_once(
        ...
        "vm testing initialisers",
    )

    if text.count("      _catalogue = result.catalogue;\n") != 2:
        raise RuntimeError("vm catalogue assignments: expected exactly two")
    text = text.replace(
...
def main() -> int:
    ...
    if args.check:
        print("configured_lists_patch_v2=present")
    else:
        print("configured_lists_patch_v2=applied")
        for path in changed:
