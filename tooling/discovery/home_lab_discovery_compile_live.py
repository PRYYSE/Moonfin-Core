#!/usr/bin/env python3
"""Compile Home Lab's Discovery catalogue against the configured live Seerr.

This script is deliberately read-only against Jellyfin and Seerr. It retrieves
existing credentials through the established Home Lab helpers without printing
them, resolves semantic keyword/provider names to current IDs, drops ambiguous
or unavailable optional lanes, and atomically writes a compiled catalogue.
...
DEFAULT_ROOT = Path("/opt/src/moonfin-dev/discovery")
DEFAULT_OUTPUT = DEFAULT_ROOT / "discovery.catalogue.compiled.json"
DEFAULT_CACHE = DEFAULT_ROOT / "semantic-resolution-cache.json"
DEFAULT_DIAGNOSTICS = DEFAULT_ROOT / "discovery.compile.diagnostics.json"
...
def request_json(method: str, path: str, token: str) -> Any:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "HomeLabDiscoveryCompiler/1.0",
        },
        method=method,
    )
    try:
    ...
    *,
    ...
    for requested_key in sorted(requirements):
        candidates = sorted(
            ...
            key=lambda value: (value != requested_key, len(value), value.lower()),
        )
        ...
        for candidate in candidates:
            ...
            # Exact duplicates with different IDs are ambiguous and must not be
            # guessed. A single exact match is safe to cache.
...
def provider_union(movie: dict[str, int], tv: dict[str, int]) -> dict[str, int]:
    ...
    for name, provider_id in tv.items():
        ...
        if existing is None:
            ...
            # Provider IDs are expected to be universal. A disagreement is
            # safer to omit than to guess.
...
def provider_available(
    ...
    return any(compiler.normalise(candidate) in selected for candidate in candidate_names)
...
def main() -> int:
    ...
    compiled["compileDiagnostics"] = {
        ...
        "compiledLaneCount": sum(len(tab["sections"]) for tab in compiled["tabs"]),
